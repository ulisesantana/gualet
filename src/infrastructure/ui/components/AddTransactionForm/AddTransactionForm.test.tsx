...
import {AddTransactionForm, AddTransactionFormProps} from './AddTransactionForm';
...
describe('AddTransactionForm', () => {
  ...
  it('updates the datalist options when operation changes', () => {
    ...
    function getCategoryOptions() {
      return Array.from(element.querySelectorAll('#category-options option') as NodeListOf<HTMLOptionElement>, (option: HTMLOptionElement) => option.value)
    }

    // Initially, it should show outcome categories in datalist
    const initialCategories = getCategoryOptions()
    expect(initialCategories).toHaveLength(mockSettings.outcomeCategories.length);
  ...
  it('calls onSubmit with correct transaction data on form submit', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Day:/i), {target: {value: '20'}});
    ...
    await waitFor(() => {
      ...
      expect(mockOnSubmit.mock.lastCall[0].description).toBe('Test transaction')
      expect(mockOnSubmit.mock.lastCall[0].type).toBe('Cash')
      expect(mockOnSubmit.mock.lastCall[0].operation).toBe(TransactionOperation.Outcome)
    });
  });
...
// TODO: Is not working for some reason
  it('resets the form after successful submission', async () => {
    ...
    await waitFor(() => {
      ...
      expect(element.querySelector('[name="operation"]')).toHaveValue(TransactionOperation.Outcome)
